...
- [ ] In `crates/hypernext-http/src/extract.rs`:
  ...
  - Pipeline:
    ...
    6. If HTML: `legible::extract(&html)` → readable content + metadata
    7. If markdown: `comrak::parse_to_html` then `legible::extract`
    ...
    12. Build `Metadata` from `<meta>` tags, JSON-LD, OG/Twitter cards, microformats (using the `microformats` crate for h-card, h-entry)
